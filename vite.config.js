...
const addTransformIndexHtml = {
	...
	transformIndexHtml(html) {
		// Only inject Horizons dev/debug scripts in development mode.
		// These scripts add significant blocking JS overhead in production
		// and directly contribute to high Total Blocking Time (TBT) scores.
		const tags = isDev ? [
			{
				tag: 'script',
				attrs: { type: 'module' },
...
export default defineConfig({
	...
	build: {
		minify: 'terser',
		// Disable modulePreload - prevents browser from eagerly fetching heavy chunks (Three.js 882KB)
		// before they are actually needed, which was the primary cause of 17,000ms TBT
		modulePreload: false,
		terserOptions: {
			compress: {
				drop_console: true,
		...
		rollupOptions: {
			...
			output: {
				// More granular code splitting so Three.js is only fetched on demand
				manualChunks(id) {
					// Three.js and its ecosystem - massive chunk, must be deferred
